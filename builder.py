...
zips = {
	"windows": "electron_zipped/electron-v43.2.0-win32-x64.zip",
	"linux": "electron_zipped/electron-v43.2.0-linux-x64.zip",
}

# To build Krobus: (for info see https://electronjs.org/docs/tutorial/application-distribution)
#
...
for key, value in zips.items():
	if not os.path.exists(value):
		continue
	build_dir = "dist/krobus-{}-{}".format(version, key)
	...
	if os.path.exists(os.path.join(build_dir, "electron.exe")):
		os.rename(os.path.join(build_dir, "electron.exe"), os.path.join(build_dir, "krobus.exe"))
	if os.path.exists(os.path.join(build_dir, "electron")):
		os.rename(os.path.join(build_dir, "electron"), os.path.join(build_dir, "krobus"))
